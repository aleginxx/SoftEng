...
router.post('/admin/upload/titleepisode', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const tempFilePath = path.join(__dirname, '../uploads', 'temp_title_episode.tsv');
        ...
        // Import data into table Episode from the temporary file
        const importEpisodeQuery = `
        ...
        try {
            await connection.query(importEpisodeQuery);
...
router.get('/admin/upload/titleepisode', (req, res) => {
