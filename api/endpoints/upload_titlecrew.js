...
router.post('/admin/upload/titlecrew', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const tempFilePath = path.join(__dirname, '../uploads', 'temp_title_crew.tsv');
        ...
        // Import data into table Crew from the temporary file
        const importCrewQuery = `
        ...
        try {
            await connection.query(importCrewQuery);
...
router.get('/admin/upload/titlecrew', (req, res) => {
